...
use crate::state::{Booking, BookingStatus, PlatformConfig, Property};

use crate::error::PropertyError;

pub fn finalize_booking(ctx: Context<FinalizeBooking>) -> Result<()> {
    ...
    let is_auto_completing =
        current_time > booking.check_out_date && booking.status == BookingStatus::Pending;
    ...
    let host_payment = booking
        .total_amount
        .checked_sub(booking.platform_fee)
        .ok_or(PropertyError::ArithmeticOverflow)?;

    let escrow_lamports = ctx.accounts.escrow_account.lamports();
    require!(
        escrow_lamports >= booking.total_amount,
        PropertyError::InsufficientEscrowFunds
    );

    let booking_key = booking.key();
    let seeds = &[b"escrow", booking_key.as_ref(), &[ctx.bumps.escrow_account]];
    ...
    if is_host_confirming {
        ...
        property.total_bookings = property
            .total_bookings
            .checked_add(1)
            .ok_or(PropertyError::ArithmeticOverflow)?;
...
pub struct FinalizeBooking<'info> {
    ...
    #[account(
        mut,
        seeds = [b"booking", property.key().as_ref(), booking.guest.key().as_ref(), &booking.check_in_date.to_le_bytes()],
        bump = booking.bump,
    )]
    ...
    /// CHECK: Platform authority account for receiving fees
    #[account(
        mut,
